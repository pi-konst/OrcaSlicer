...
TEST_CASE("SlicingPipeline hook fires once per step per object in order", "[slicing_pipeline]") {
    ...
    CHECK(count(S::PrepareInfill) == 1);   // G4: the prepare-infill seam fires once per object
    ...
    CHECK(idx(S::Perimeters) < idx(S::PrepareInfill)); // G4: prepare-infill fires after perimeters...
